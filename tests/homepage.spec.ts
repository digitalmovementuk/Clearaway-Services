...
test.describe('Clearaway homepage preview', () => {
  test('core content, layout and accessibility pass', async ({ page }) => {
    ...
    const heroVideo = hero.getByTestId('hero-video')
    await expect(heroVideo).toBeVisible()
    const heroBox = await hero.boundingBox()
    const imageBox = await heroVideo.boundingBox()
    ...
    await expect
      .poll(async () => heroVideo.evaluate((video) => (video as HTMLVideoElement).currentTime), { timeout: 5000 })
      .toBeGreaterThan(0.1)
  ...
  test('mobile layout, sticky CTA and screenshots', async ({ page }) => {
    ...
    expect(videoBox?.width).toBeGreaterThanOrEqual(heroBox?.width ?? 0)
    expect(videoBox?.height).toBeGreaterThanOrEqual(heroBox?.height ?? 0)
    await page.waitForTimeout(1200)
    await expect(heroVideo).toHaveJSProperty('paused', false)
